...
if(isset($_POST["transition"]) && $_POST["transition"]) {
	...
	if (!is_object($transition)) {
		(new UI($GLOBALS['theme'] ?? 'bootstrap'))->exitError(getMLText("document_title", array("documentname" => $document->getName())),getMLText("invalid_workflow_transition"));
	}
} else {
	$transition = null;
}
...
$view = (new UI($GLOBALS['theme'] ?? 'bootstrap'))->factory($theme, $tmp[1], array('dms'=>$dms, 'user'=>$user, 'folder'=>$folder, 'document'=>$document, 'version'=>$version, 'transition'=>$transition));
if($view) {
	$view->show();
	exit;
}
