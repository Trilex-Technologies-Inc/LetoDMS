...
class LetoDMS_Core_User {
	...
	function getRoles() {
		...
		if(!is_array($rows)) return false; $roles=array(); foreach($rows as $row) { $role=new LetoDMS_Core_Role($row['id'],$row['name'],$row['description']); $role->setDMS($this->_dms); $roles[]=$role; } return $roles;
	}

	function hasPermission($permission) {
		if($this->isAdmin()) return true; if(!is_string($permission)||$permission==='') return false; $db=$this->_dms->getDB();
		$rows=$db->getResultArray("SELECT p.id FROM tblPermissions p INNER JOIN tblRolePermissions rp ON rp.permissionID=p.id INNER JOIN tblUserRoles ur ON ur.roleID=rp.roleID WHERE ur.userID=".(int)$this->_id." AND p.name=".$db->qstr($permission));
